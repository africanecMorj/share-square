...
let frameIndex2 = 0;
let frameDelay2 = 0;
const totalFrames2 = 2;

let frameIndex = 0;
let frameDelay = 0;
let frameDelay1 = 0;
let frameIndex1 = 0;
const totalFrames1 = 33;
...
function updateCamera() {
  ...
  if (playerScreenX > screenWidth) {
    ...
    localStorage.setItem(`currentCameraX`,currentCameraX);
...
function loadLevel(level) {
  ...
  game.appendChild(npcIndicator);
   ...
   const npc2 = document.createElement('div');
  npc2.id = 'npc2';
  npc2.style.position = 'absolute';

  
  npc2.transition = `0`
  // npc.style.background = 'blue';
  game.appendChild(npc2);


  const npcIndicator2 = document.createElement('div');
  npcIndicator2.id = 'npcIndicator2';
  npcIndicator2.textContent = '[E]';
  npcIndicator2.style.position = 'absolute';
  npcIndicator2.style.display = 'none';
  npcIndicator2.style.fontWeight = 'bold';
  game.appendChild(npcIndicator2);
...
const npcIndicator1 = document.createElement('div');
  ...
  game.appendChild(fileList);
    ...
    const savedCamera = parseFloat(localStorage.getItem('currentCameraX'));
          
          if (!isNaN(savedCamera)) currentCameraX = savedCamera;
 ...
 const dialogLines = [
      ...
      { text: `На щастя, ти тут не в перше. Натисни на esc і зазирни в історію, в ній все написано`,  },
    ];
const dialogLines1 = [
      { text: "Код?", choices: ["Ввести", "Я не пам'ятаю"]},
      { text: " " },
...
const dialogLines3 = [
  ...
  const dialogLines10 = [
      { text: "Хай славится великий та могутній esc"},
      { text: "Відчуй силу та владу великого esc" },
  ...
  typeSound.play().catch((err) => {
    ...
    function showNextLine() {
      ...
      if (line.choices) {
        ...
        document.getElementById(`dialogImage`).style.backgroundImage = `url(./sprites/chesterDialog.png)`;        
        currentLine++;
        if (currentLine >= dialogLines10.length) {
        moveVerif = true;  
        dialogBox.textContent = "";
        dialogContainer.style.opacity = `0%`
        choicesDiv.style.opacity = `0%`
      ...
      const line = dialogLines10[currentLine];
      if (line.choices) {
        typeLine(line.text);
        const waitUntilTyped = setInterval(() => {
          if (!isTyping) {
...
document.addEventListener('keydown', e => {
  ...
  if(e.keyCode == 27 && loadVerif){
    moveVerif = false;
    menuVerif = false;
    globMusic.volume = 0.015;
    $(`.menu`).fadeIn(100);
    game.style.filter = `brightness(5%)`;
  }

  if (e.keyCode === 69 && menuVerif) keys.e = true;
...
function update() {
  ...
  const npcIndicator2 = document.getElementById(`npcIndicator2`);
...
if(npc2 && npcIndicator2){
     if(dialogueVerif == true){
      npcIndicator2.style.display = 'none';
     }else{
    const npcX = npc2.offsetLeft;
    const npcY = npc2.offsetTop;
    const distance = Math.abs(npcX - posX);
    if (distance < 200) {
      npcIndicator2.style.display = 'block';
      // npcIndicator.style.left = (npcX / gameWidth) * 100 + '%';
      // npcIndicator.style.top = (npc.offsetTop - 20) + 'px';
      // console.log(keys.e)
      if (keys.e == true) {
  ...
  frameDelay1++;
    if (frameDelay1 > 30) {
      frameIndex1 = (frameIndex1 + 1) % totalFrames1;
      frameDelay1 = 0;
    }
    ...
    if(npc){
      if(frameIndex1 == 0){
    npc.style.backgroundImage = `url(./sprites/bardCalm.png)`;

    }else{
    npc.style.backgroundImage = `url(./sprites/bardCalm${frameIndex1}.png)`;
    }
    }

     frameDelay2++;
    if (frameDelay2 > 50) {
      frameIndex2 = (frameIndex2 + 1) % totalFrames2;
      frameDelay2 = 0;
    }
    ...
    if(npc2){
      if(frameIndex2 == 0){
    npc2.style.backgroundImage = `url(./sprites/chesterCalm1.png)`;

    }else{
    npc2.style.backgroundImage = `url(./sprites/chesterCalm2.png)`;
    }
    }
  ...
  if(frameIndex == 1) {
    ...
    if(globMusic.currentTime >= globMusic.duration - 0.05) {
		globMusic.currentTime = 0;
		globMusic.play();
	}
 ...
 localStorage.setItem('playerX', posX);
localStorage.setItem('playerY', posY);
  updateCamera();
...
.then(res => {
  currentUserHistory = res.data.history;
   let uniqueHistory = Array.from(
  new Map(currentUserHistory.map(item => [item.code, item])).values()
    )
...
let dialogLines4 = [
  {text:`Потрібно перенести файли з провідника в скриньку`},
  {text: `Код: ...`},
  {text:` `},
    ...
    if (e.key === 'Enter') {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          loadLevel(currentLevel);
    const savedX = parseFloat(localStorage.getItem('playerX'));
          const savedY = parseFloat(localStorage.getItem('playerY'));
          if (!isNaN(savedX)) posX = savedX;
          if (!isNaN(savedY)) posY = savedY;

          // 3. Встановлюємо позицію гравця
   ...
   if(e.target.id.substring(0, 3) == `upl`){
    ...
    axios.get(`http://localhost:3000/download/${e.target.id.substring(3)}`,  {responseType: 'blob'})
        .then(res => {
            ...
            showPopup(`Файл завантажено`);
          
        })
        .catch(err => {  
            showPopup(`Неперебдачені проблеми, спробуйте пізніше`);
